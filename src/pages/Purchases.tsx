...
import { Plus, Eye, IndianRupee, Calendar, Filter, FileDown, Printer } from "lucide-react";
...
export default function Purchases() {
  ...
  const summary = data?.summary || { totalAmount: 0, paidAmount: 0, dueAmount: 0 };
              ...
              <tbody className="divide-y divide-gray-50">
                {(data?.purchases || []).map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="px-3 py-2.5">
                      <button onClick={() => navigate(`/purchases/${p.id}`)} className="p-1.5 rounded hover:bg-blue-50 text-blue-600" title="View">
              ...
              <div key={p.id} className="p-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => navigate(`/purchases/${p.id}`)} className="p-1.5 rounded bg-blue-50 text-blue-600"><Eye className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              </div>
            ))}
